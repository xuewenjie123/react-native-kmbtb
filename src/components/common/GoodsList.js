...
export default class GoodsList extends Component {
    ...
    componentDidMount(){
      _this.setState({
        dataSource:new ListView.DataSource({rowHasChanged: (r1, r2) =>r1!== r2}).cloneWithRows(this.props.goodsList),
        goodsList:this.props.goodsList
      })
    }
    componentWillReceiveProps(newProps){
      if(_state.goodsList!=newProps.goodsList){
        _this.setState({
          dataSource:new ListView.DataSource({rowHasChanged: (r1, r2) =>r1!== r2}).cloneWithRows(newProps.goodsList),
    ...
    _renderHeader(){
      return (
         <View style={styles.header_bar}>
             ...
             <View style={{flex:1,alignItems:"center",}}>
               <Text style={{fontSize:20,color:"#222"}} numberOfLines={1}>选择货品</Text>
             </View>
     ...
     confim(rowContent,rowID){
      _this.setState({
        selectIndex:rowID,
        itemContent:rowContent.good_name,
        dataSource:new ListView.DataSource({rowHasChanged: (r1, r2) =>r1!== r2}).cloneWithRows(this.props.goodsList)
      })
        _this.props.confirm(rowContent)
     }
     _renderRow(rowContent,sectionID,rowID){
       return (
         <TouchableOpacity style={styles.info_a1} key={rowID} onPress={() => {_this.confim(rowContent,rowID)}}>
            <Text style={_state.selectIndex==rowID?text.lan12:text.hei12}>{rowContent.good_name}</Text>
         </TouchableOpacity>
       )
     }
